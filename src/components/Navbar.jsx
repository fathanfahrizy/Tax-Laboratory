...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          // Hanya hitung progress jika layar digeser
    ...
    // Tambahkan passive: true agar browser tidak menunda scroll HP
    window.addEventListener("scroll", handleScroll, { passive: true });
    handleScroll(); // Panggilan pertama
  ...
  return (
    <div className={`fixed w-full z-[100] flex justify-center pointer-events-none transition-all duration-300 ease-out ${isScrolled ? 'top-4 md:top-5 px-4 sm:px-6' : 'top-0 px-0'}`}>
      <nav 
        ref={navRef} 
      ...
      >
        <div className="w-full flex items-center justify-between relative z-20">
          ...
          <div className={`flex justify-start transition-all duration-300 ease-out ${isScrolled ? 'flex-1' : 'w-auto'}`}>
            <Link to="/" onClick={closeAll} className="flex items-center gap-3 sm:gap-4 shrink-0 group">
              <div className={`bg-white rounded-full flex items-center justify-center overflow-hidden border border-slate-100 shadow-sm transition-all duration-300 group-hover:scale-105 group-hover:shadow-md shrink-0
              ...
              <div className="flex flex-col justify-center pt-0.5">
                <span className={`font-black tracking-tight leading-none transition-colors duration-300 
                ...
                <span className={`font-bold tracking-[0.25em] uppercase transition-colors duration-300 
          ...
          {/* === DESKTOP NAVIGATION === */}
          <div className={`hidden lg:flex gap-10 font-bold text-base lg:text-lg items-center transition-all duration-300 ease-out ${isScrolled ? 'justify-center shrink-0' : 'flex-1 justify-end'}`}>
            ...
            <div className="relative group/item">
              ...
              {activeDropdown === 'profil' && (
                <div className="absolute left-1/2 -translate-x-1/2 mt-7 w-64 bg-white/95 backdrop-blur-xl border border-gray-100 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)] rounded-2xl overflow-hidden py-3 animate-in fade-in slide-in-from-top-4 duration-200 ring-1 ring-black/5">
        ...
        {/* === MOBILE OVERLAY MENU (OPTIMASI ANTI LAG) === */}
        <div className={`lg:hidden absolute top-full left-0 w-full overflow-hidden transition-[max-height,opacity] duration-300 ease-in-out bg-white shadow-2xl rounded-b-[2.5rem] transform-gpu will-change-transform ${isMobileMenuOpen ? 'max-h-[85vh] opacity-100 border-t border-gray-100' : 'max-h-0 opacity-0'}`}>
          <div className="flex flex-col gap-2 pb-8 font-bold text-gray-700 overflow-y-auto px-5 pt-5 text-lg md:text-xl">
            ...
            <div className="flex flex-col">
              ...
              <div className={`flex flex-col overflow-hidden transition-[max-height,opacity] duration-300 ease-in-out ${activeDropdown === 'm-profil' ? 'max-h-80 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                <div className="bg-gray-50 rounded-2xl mx-2 flex flex-col py-3 border border-gray-100">
            ...
            <div className="flex flex-col">
              ...
              <div className={`flex flex-col overflow-hidden transition-[max-height,opacity] duration-300 ease-in-out ${activeDropdown === 'm-layanan' ? 'max-h-80 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                <div className="bg-gray-50 rounded-2xl mx-2 flex flex-col py-3 border border-gray-100">
            ...
            <div className="flex flex-col">
              ...
              <div className={`flex flex-col overflow-hidden transition-[max-height,opacity] duration-300 ease-in-out ${activeDropdown === 'm-informasi' ? 'max-h-80 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                <div className="bg-gray-50 rounded-2xl mx-2 flex flex-col py-3 border border-gray-100">
